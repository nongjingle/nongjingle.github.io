---
title: "assignment1 of cs106l-sp25"
categories: [cs106l-sp25]
tags: [cs106l-sp25]
---
## assignment1 of cs106l-sp25
```cpp
/*
...
void parse_csv(const std::string& filename, std::vector<Course>& courses) {
  ...
  while (std::getline(ifs, line)) {
    ...
    courses.push_back({str_vec[0], str_vec[1], str_vec[2]});
  }
  ifs.close();
}
...
void write_courses_offered(std::vector<Course>& all_courses) {
  /* (STUDENT TODO) Your code goes here... */
  std::ofstream ofs(COURSES_OFFERED_PATH, std::ios::out | std::ios::trunc);
  ofs << "Title,Number of Units,Quarter" << std::endl;
  size_t len = all_courses.size();
  size_t it = 0;
  while (it < len) {
    auto course = all_courses[it];
...
void write_courses_not_offered(const std::vector<Course>& unlisted_courses) {
  /* (STUDENT TODO) Your code goes here... */
  std::ofstream ofs(COURSES_NOT_OFFERED_PATH, std::ios::out | std::ios::trunc);
  ofs << "Title,Number of Units,Quarter" << std::endl;
